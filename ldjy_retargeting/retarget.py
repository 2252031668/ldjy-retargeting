"""Unified retargeting interface for the LDJY hand.
...
Usage:
    retargeter = Retargeter.from_yaml("example/config/adaptive_analytical_video.yaml", hand_side="right")
...
class Retargeter:
    ...
    def __init__(self, config: dict, hand_side: str = "right"):
        ...
        # Optional device-specific residual correction in the MANO/asset task
        # frame. The fixed MANO -> LDJY alignment belongs to generated assets.
    ...
    def _prepare_keypoints(self, raw_keypoints: np.ndarray) -> np.ndarray:
        """Convert raw landmarks into the MANO-aligned LDJY task frame."""
