...
class BaseOptimizer(ABC):
    ...
    def __init__(self, config: dict):
        ...
        # Load the selected URDF before resolving semantic link names, so an
        # override and its link_naming are always interpreted against the same hand.
        ...
        if urdf_override:
            ...
            urdf_path = str(
                (
                    ...
                    / "ldjy_hand"
                    / "urdf"
                    / f"ldjy_{self.hand_side}_hand.urdf"
                ).resolve()
            )
        ...
        # Link names (resolved from config; defaults target the bundled LDJY hand).
    ...
    # Default semantic frames for the bundled LDJY URDF. The fingers list is in
    # MediaPipe order: thumb, index, middle, ring, pinky.
    ...
    def _resolve_link_names(self, config: dict):
        ...
        ``optimizer.link_naming`` maps the optimizer's logical roles (wrist and
        per-finger tip / PIP / DIP / link1) onto the names used by another URDF.
        Omitting the block uses the bundled LDJY hand naming.
    ...
    def _get_init_qpos(self, last_qpos: Optional[np.ndarray]) -> np.ndarray:
        ...
        if last_qpos is not None:
            ...
            init_qpos = np.zeros(self.num_joints, dtype=np.float64)
    ...
    def _run_optimization(self, objective_fn, init_qpos: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            qpos = np.asarray(init_qpos, dtype=np.float64)
        qpos = np.clip(
            qpos,
            self.robot.joint_limits[:, 0],
            self.robot.joint_limits[:, 1],
