...
class TuningSessionTests(unittest.TestCase):
    ...
    def test_first_save_creates_immutable_original_backup(self):
        ...
        session.set_value("retarget.scaling", 1.2)
        session.save()
        ...
        session.set_value("retarget.scaling", 1.3)
        session.save()

        self.assertEqual(first_bytes, original.read_bytes())
        self.assertEqual(
            yaml.safe_load(self.config_path.read_text(encoding="utf-8"))["retarget"]["scaling"],
            1.3,
        )

    def test_restore_default_only_changes_memory_until_save(self):
        session = TuningSession(self.config_path)
        session.set_value("retarget.scaling", 0.8)
        session.save()

        session.restore_default()

        self.assertTrue(session.is_dirty)
        self.assertEqual(session.config["retarget"]["scaling"], 1.0)
        self.assertEqual(
            yaml.safe_load(self.config_path.read_text(encoding="utf-8"))["retarget"]["scaling"],
            0.8,
        )
